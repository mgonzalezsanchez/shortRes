...
def shortRes(r,I,printlevel=0):
    ...
    A minimal set of generators of r/I as A-module and a list containing the whole
    resolution, if r/I is not Cohen-Macaulay.
    When printlevel=1, the function prints the Betti diagram and the regularity 
    of r/I when it is standard graded.
    The function returns an error if the ideal is not homogeneous or the variables 
    are not in Noether position.

    EXAMPLES:: 
 
        sage: A = numpy.array([[7,2,3],[1,8,3],[3,8,1],[12,0,0],[0,12,0],[0,0,12]])
        sage: r,I = toricIdeal(A,QQ)
        sage: B0,res = shortRes(r,I,1)
    ...
    if I.is_homogeneous() == 0:
        raise ValueError('The ideal is not homogeneous')
    # We change the monomial order in r just in case it's not wdegrevlex
    ...
    if kdim == Infinity:
        raise ValueError('The variables are not in Noether position')
    ...
    # If H is empty, r/I is Cohen-Macaulay, and we return the minimal 
    # generating set of r/I as A-module (B0)
    if len(H) == 0:
        if printlevel == 1 and w == tuple(len(r.gens())*[1]):
            reg = max([g.degree() for g in B0])
            print('reg(R/I) = '+str(reg))
        return B0, []
...
def schreyerResDim3(r,I):
    ...
    resolution.
    The function returns an error if the ideal is not homogeneous, not toric (prime and 
    binomial), the dimension of r/I is not 3, or the variables are not in Noether position.
    ...
    x = r.gens(); n = len(x); d = I.dimension()
    if I.is_homogeneous() == 0:
        raise ValueError('I is not homogeneous')
    if d != 3:
        raise ValueError('dim(R/I) != 3')
    if I.is_prime() == 0:
        raise ValueError('I is not prime')
        
    G = I.groebner_basis()
    lG = [len(g.monomials()) for g in G]
    if lG != len(lG)*[2]:
        raise ValueError('I is not binomial')
    gen_inI = [g.lm() for g in G]; inI = ideal(gen_inI)
